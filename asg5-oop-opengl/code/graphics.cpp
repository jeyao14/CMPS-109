...
int window::movement_amount = 4;
int window::border_width = 4;
string window::border_colour = "red";
mouse window::mus;
...
void object::move (GLfloat delta_x, GLfloat delta_y) {
   ...
   if(center.xpos > wwidth){
      center.xpos = 0;
   }else if(center.xpos < 0){
      center.xpos = wwidth;
   }

   if(center.ypos > wheight){
      center.ypos = 0;
...
void window::keyboard (GLubyte key, int x, int y) {
   ...
   switch (key) {
      ...
      case 'H': case 'h':
         ...
         window::objects.at(selected_obj).move(-window::movement_amount,0);
         break;
      case 'J': case 'j':
         //move_selected_object (
         window::objects.at(selected_obj).move(0,-window::movement_amount);
         break;
      case 'K': case 'k':
         //move_selected_object (
         window::objects.at(selected_obj).move(0,window::movement_amount);
